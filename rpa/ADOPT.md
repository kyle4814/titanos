...
human_jurisdiction.py`, which this session found and closed a live
exploit in. Reuses `magl/`'s validator pattern and `kpm/promotion/
...
from rpa.gates.human_jurisdiction import authorize_pilot, confirm_pilot_authorized
...
- **In scope:** forged authorization history (closed — see above),
  ...
  by re-deriving from history, not trusting `.state`).
...
No persistence layer. No cryptographic signing.
...
- 2026-08-25: `PromotionRecord.history` frozen — closed the live
  ...
  against.
- `rpa/`'s own suite: 207 tests as of 2026-08-26.
