...
from foundation.spec_crossref import (  # noqa: E402
    TermCoverage, format_term_coverage, trace_term,
)


class TestAssumptionTraceability(unittest.TestCase):
    """Measured against Swiss Post's real corpus 2026-09-04: 'SGSP'
    appears 43 times in the computational proof and 20 times in the
    external analysis of it, and ZERO times in the System Specification
    and Verifier Specification -- the two documents that define and
    verify the seed-format control which delivers the assumption.

    Not a vulnerability. A load-bearing property documented as a naming
    convention, which is the shape of thing a later revision relaxes.
    """

    DOCS = {
        "proof": "The privacy proof relies on the SGSP problem. SGSP is hard.",
        "external note": "About the SGSP problem, by Gaudry.",
        "system spec": "The input seed is restricted to the format CT_YYYYMMDD_XYnm.",
        "verifier spec": "The seed value must be of the correct format.",
    }

    def test_absence_is_reported_per_document(self):
        c = trace_term("SGSP", self.DOCS)
        self.assertEqual(c.absent_from, ("system spec", "verifier spec"))
        self.assertEqual(c.present_in, ("external note", "proof"))

    def test_uneven_coverage_is_its_own_status(self):
        self.assertEqual(trace_term("SGSP", self.DOCS).status, "UNEVEN")

    def test_present_everywhere_is_not_flagged(self):
        c = trace_term("seed", {"a": "the seed", "b": "a seed again"})
        self.assertEqual(c.status, "PRESENT_EVERYWHERE")
        self.assertEqual(c.absent_from, ())

    def test_absent_everywhere_is_distinguished_from_uneven(self):
        c = trace_term("quantum", self.DOCS)
        self.assertEqual(c.status, "ABSENT_EVERYWHERE")

    def test_aliases_are_counted_too(self):
        docs = {"spec": "relies on the Subgroup Generated by Small Primes"}
        bare = trace_term("SGSP", docs)
        with_alias = trace_term("SGSP", docs,
                                aliases=("Subgroup Generated by Small Primes",))
        self.assertEqual(bare.mentions["spec"], 0)
        self.assertEqual(with_alias.mentions["spec"], 1)

    def test_a_term_split_by_a_ligature_is_still_found(self):
        """The failure this whole module exists to prevent: a term the
        checker cannot see is a term it reports as absent, and absence
        is the finding shape here."""
        c = trace_term("verification", {"spec": "the veriﬁcation of the seed"})
        self.assertEqual(c.mentions["spec"], 1)

    def test_matching_is_case_insensitive(self):
        c = trace_term("sgsp", {"d": "The SGSP Problem"})
        self.assertEqual(c.mentions["d"], 1)

    def test_the_render_states_absence_is_not_proof(self):
        """A sibling specification not read here may carry the
        rationale. UNKNOWN, not zero -- the same rule as everywhere."""
        out = format_term_coverage(trace_term("SGSP", self.DOCS))
        self.assertIn("UNKNOWN, not zero", out)
        self.assertIn("THE DOCUMENTS SUPPLIED", out)

    def test_no_documents_is_not_assessed(self):
        c = trace_term("SGSP", {})
        self.assertEqual(c.status, "NOT_ASSESSED")
        self.assertIn("NOT_ASSESSED", format_term_coverage(c))

    def test_an_empty_term_is_refused(self):
        with self.assertRaises(SpecCrossRefError):
            trace_term("  ", {"a": "text"})

    def test_a_non_text_document_is_refused(self):
        with self.assertRaises(SpecCrossRefError):
            trace_term("SGSP", {"a": 42})

    def test_format_refuses_a_non_coverage(self):
        with self.assertRaises(SpecCrossRefError):
            format_term_coverage("UNEVEN")
