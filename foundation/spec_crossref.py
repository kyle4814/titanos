...
#
# A security proof rests on named assumptions. The documents that
# IMPLEMENT those assumptions have to say which control protects which
# assumption, or the control looks like a convention and the next
# revision relaxes it.
#
# Found by reading Swiss Post's e-voting corpus 2026-09-04: the privacy
# proof rests on the Subgroup Generated by Small Primes (SGSP) problem,
# whose external analysis states that the prime must be "provably
# randomly generated". The control that delivers that is a restriction
# on the seed format. "SGSP" appears ZERO times in the 166-page System
# Specification and ZERO times in the 48-page Verifier Specification,
# and the format restriction is stated with no security rationale
# attached.
#
# That is not a vulnerability. It is a load-bearing property documented
# as a naming convention, which is exactly the shape of thing that gets
# relaxed by someone who does not know what it is holding up.
# ---------------------------------------------------------------------


@dataclass(frozen=True)
class TermCoverage:
    """Where a named term appears across a corpus, and where it does
    not. `absent_from` is the interesting half."""
    ...
    def present_in(self) -> Tuple[str, ...]:
        return tuple(sorted(d for d, n in self.mentions.items() if n > 0))

    @property
    def absent_from(self) -> Tuple[str, ...]:
        return tuple(sorted(d for d, n in self.mentions.items() if n == 0))
    ...
    def status(self) -> str:
        ...
        if not self.absent_from:
            return "PRESENT_EVERYWHERE"
        return "UNEVEN"


def trace_term(term: str, documents: Dict[str, str], *,
               aliases: Optional[Tuple[str, ...]] = None) -> TermCoverage:
    """Count mentions of `term` (and any `aliases`) in each named
    document.

    Case-insensitive, and normalised first -- a term split by a ligature
    or a soft hyphen is a term this would otherwise report as absent,
    which is the exact false conclusion the rest of this module exists
    to avoid.
    """
    if not isinstance(term, str) or not term.strip():
        raise SpecCrossRefError("trace_term() needs a non-empty term")
    if not isinstance(documents, dict):
        raise SpecCrossRefError(
            "trace_term() takes {document_name: text}, got "
            f"{type(documents).__name__}")

    needles = [term] + list(aliases or ())
    mentions: Dict[str, int] = {}
    for name, text in documents.items():
        if not isinstance(text, str):
            raise SpecCrossRefError(
                f"document {name!r} is not text ({type(text).__name__})")
        haystack = normalise_extracted_text(text).lower()
        mentions[name] = sum(
            haystack.count(normalise_extracted_text(n).lower())
            for n in needles)
    return TermCoverage(term=term, mentions=mentions)


def format_term_coverage(coverage: TermCoverage) -> str:
    """Render, leading with absence, because absence is the finding
    shape -- and saying plainly that absence here is not proof the
    rationale is undocumented, only that it is not in what was read."""
    if not isinstance(coverage, TermCoverage):
        raise SpecCrossRefError(
            "format_term_coverage() takes a TermCoverage, not "
            f"{type(coverage).__name__}")
    if coverage.status == "NOT_ASSESSED":
        return f"NOT_ASSESSED -- no documents supplied for {coverage.term!r}."

    lines = [f"TERM TRACE: {coverage.term!r}", ""]
    for name in sorted(coverage.mentions):
        n = coverage.mentions[name]
        mark = "  " if n else "!!"
        lines.append(f"  {mark} {name}: {n} mention(s)")
    lines.append("")
    if coverage.status == "UNEVEN":
        lines.append(
            f"ABSENT FROM {len(coverage.absent_from)} of "
            f"{len(coverage.mentions)} documents: "
            f"{', '.join(coverage.absent_from)}")
        lines.append(
            "A term that names a security assumption, absent from the "
            "document that implements the control protecting it, means the "
            "control reads as a convention to whoever revises it next.")
    lines.append(
        "Absence here is absence from THE DOCUMENTS SUPPLIED. It is not "
        "proof the rationale is undocumented -- a sibling specification "
        "not read here may carry it. UNKNOWN, not zero.")
    return "\n".join(lines)
